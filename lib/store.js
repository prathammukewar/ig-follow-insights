...
//   throttle            { until, at, source }  set when Instagram rate limits; the dashboard warns until it passes
...
export const DEFAULT_SETTINGS = {
  pageSize: 200,       // largest page size to ask for (following); the scan steps down if Instagram refuses
  followersPageSize: 50, // followers come back slowly at bigger sizes
  delayMin: 800,       // ms between requests to start with (adaptive pacing speeds up and backs off from here)
  delayMax: 5000,      // ms cap when Instagram pushes back
  parallelLists: true, // fetch followers and following at the same time
  scanWorkers: 3,      // pages fetched at once per list when Instagram's cursor is a plain offset
  scanPreset: 'normal',
  bioPreset: 'normal',
  dailyActionCap: 60,  // follow + unfollow actions allowed per rolling 24 hours
  ...
  bioDelay: 800,       // ms between profile (bio) requests
  ...
  waitUnknown: true,   // waiting room: include people followed before the first scan (exact date unknown)
...
// Speed presets. Gentle stays well under Instagram's limits; fast is for small accounts.
export const SCAN_PRESETS = {
  gentle: { delayMin: 1200, delayMax: 8000, scanWorkers: 1, parallelLists: false },
  normal: { delayMin: 600, delayMax: 5000, scanWorkers: 3, parallelLists: true },
  fast: { delayMin: 500, delayMax: 5000, scanWorkers: 5, parallelLists: true },
};
export const BIO_PRESETS = {
  gentle: { bioDelay: 1500, bioConcurrency: 1 },
  normal: { bioDelay: 800, bioConcurrency: 2 },
  fast: { bioDelay: 500, bioConcurrency: 3 },
};

// Which accounts to load profiles for first: the ones you act on.
export function prioritizeForProfiles(pks, followers, following, whitelist = new Set()) {
  const f = followers instanceof Set ? followers : new Set(followers);
  const g = following instanceof Set ? following : new Set(following);
  const rank = (pk) => {
    const isF = f.has(pk), isG = g.has(pk);
    if (isG && !isF) return whitelist.has(pk) ? 3 : 0;   // not following back
    if (isF && !isG) return 1;                            // fans
    if (isF && isG) return 2;                             // mutual
    return 4;
  };
  return pks.slice().sort((a, b) => rank(a) - rank(b));
}

export const ACTION_KEY = (id) => `actions_${id}`;

// Rolling 24 hour count of follow and unfollow actions taken through the extension.
export async function actionBudget(uid, cap) {
  const key = ACTION_KEY(uid);
  const g = await chrome.storage.local.get(key);
  const cutoff = Date.now() - 24 * 3600 * 1000;
  const list = (g[key] || []).filter((t) => t > cutoff);
  if (list.length !== (g[key] || []).length) await chrome.storage.local.set({ [key]: list });
  return { used: list.length, cap, left: Math.max(0, cap - list.length), oldest: list[0] || null };
}

export async function recordAction(uid) {
  const key = ACTION_KEY(uid);
  const g = await chrome.storage.local.get(key);
  const cutoff = Date.now() - 24 * 3600 * 1000;
  const list = (g[key] || []).filter((t) => t > cutoff);
...
export async function importDataExport(uid, { followers = [], following = [], pending = [] }) {
  ...
  if (accounts[uid]) accounts[uid].exportImport = { at: Date.now(), matchedF, matchedG, totalF: followers.length, totalG: following.length, pending: pending.length };
  const patch = { [KEYS.users(uid)]: users, [KEYS.accounts]: accounts };
  if (pending.length) patch[`pending_${uid}`] = pending.map(({ u, ts }) => ({ u, ts, pk: byName.get(u.toLowerCase()) || null }));
  await chrome.storage.local.set(patch);
  return { matchedF, matchedG, updated, totalF: followers.length, totalG: following.length, pending: pending.length };
}
